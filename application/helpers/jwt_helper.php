...
class jwt_helper
{
    ...
    public static function VerifyToken($headers) {
        //response should include  code, X_AUTH_TOKEN, msg
        $response=[];
        $CI =& get_instance();
        ...
        if(!isset($headers["X_AUTH_TOKEN"]) || empty($headers["X_AUTH_TOKEN"])){
            ...
            $response = array("code" => 4,"X_AUTH_TOKEN" => $headers["X_AUTH_TOKEN"], "msg" =>"Token Unavailable");            
             ...
             $user = static::decode($headers["X_AUTH_TOKEN"],"M3g@AL13nH@sH",array('HS256'));
            if($user == "Expired token"){
                $response = array("code" => 3,"X_AUTH_TOKEN" => $headers["X_AUTH_TOKEN"], "msg" =>"Expired token");
            }else{                
    ...
    public static function decode($jwt, $key, array $allowed_algs = array())
    {
        ...
        $tks = explode('.', $jwt);
        if (count($tks) != 3) {
            $msg ="Incomplete Token:Wrong number of segments";
            return $msg;
